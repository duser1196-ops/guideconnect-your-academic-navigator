...
import { ArrowLeft, Send, User, FolderKanban, MessageSquare, Check } from "lucide-react";
...
const facultyList = [
  { id: "1", name: "Dr. Ramesh Kumar", department: "CSE", slots: 3 },
  { id: "2", name: "Dr. Meena Sharma", department: "CSE", slots: 2 },
  { id: "3", name: "Dr. Anil Verma", department: "ECE", slots: 1 },
  { id: "4", name: "Dr. Priya Singh", department: "CSE", slots: 4 },
  { id: "5", name: "Dr. Kavita Joshi", department: "IT", slots: 2 },
  { id: "6", name: "Dr. Sanjay Patel", department: "CSE", slots: 0 },
];

const projectList = [
  { id: "1", title: "ML in Healthcare" },
  { id: "2", title: "NLP Chatbot" },
  { id: "3", title: "IoT Smart Campus" },
  { id: "4", title: "Blockchain Auth System" },
  { id: "5", title: "AR Learning Platform" },
  { id: "6", title: "Data Viz Dashboard" },
];
    ...
    return (
      <div className="max-w-lg mx-auto text-center py-20">
        <motion.div
          ...
          className="mx-auto mb-6 h-20 w-20 rounded-full gradient-primary flex items-center justify-center"
        >
          <Check className="h-10 w-10 text-primary-foreground" />
        </motion.div>
        <motion.h2 initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="font-display text-2xl font-bold mb-2">
          Request Sent Successfully!
        </motion.h2>
        <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.5 }} className="text-muted-foreground">
  ...
  return (
    <div className="max-w-2xl mx-auto">
      <div className="flex items-center gap-3 mb-6">
        <Button variant="ghost" size="icon" onClick={() => navigate("/dashboard/requests")}>
          ...
          <motion.div key={step} variants={slideVariants} initial="enter" animate="center" exit="exit" transition={{ duration: 0.25 }}>
            {step === 0 && (
              <div className="space-y-4">
                ...
                <div className="grid gap-2">
                  {facultyList.map((f) => (
                    <motion.button
                      ...
                      whileTap={{ scale: 0.99 }}
                      onClick={() => { setForm({ ...form, facultyId: f.id }); setErrors({}); }}
                      disabled={f.slots === 0}
                      className={`w-full flex items-center gap-3 p-3 rounded-lg border text-left transition-colors ${
                        form.facultyId === f.id
                          ? "border-primary bg-primary/5"
                          : "border-border hover:bg-muted/50"
                      } ${f.slots === 0 ? "opacity-50 cursor-not-allowed" : ""}`}
                    >
                      <div className="gradient-primary rounded-full h-9 w-9 flex items-center justify-center shrink-0">
                        <User className="h-4 w-4 text-primary-foreground" />
                      </div>
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-medium">{f.name}</p>
                        <p className="text-xs text-muted-foreground">{f.department} · {f.slots} slots available</p>
                      </div>
                      {form.facultyId === f.id && (
                ...
                {errors.facultyId && (
                  <motion.p initial={{ opacity: 0, y: -4 }} animate={{ opacity: 1, y: 0 }} className="text-xs text-destructive">{errors.facultyId}</motion.p>
                )}
              </div>
            )}
            ...
            {step === 1 && (
              <div className="space-y-4">
                ...
                {errors.projectId && (
                  <motion.p initial={{ opacity: 0, y: -4 }} animate={{ opacity: 1, y: 0 }} className="text-xs text-destructive">{errors.projectId}</motion.p>
                )}
              </div>
            )}
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <Textarea
                    id="msg"
                    rows={5}
                    placeholder="Dear Professor, I am interested in working under your guidance because…"
                    value={form.message}
                    onChange={(e) => { setForm({ ...form, message: e.target.value }); setErrors({}); }}
                    className={errors.message ? "border-destructive" : ""}
                  />
                  {errors.message && (
                    <motion.p initial={{ opacity: 0, y: -4 }} animate={{ opacity: 1, y: 0 }} className="text-xs text-destructive">{errors.message}</motion.p>
                  )}
